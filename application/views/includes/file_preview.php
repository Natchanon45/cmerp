<?php
    if (0 && ( !get_setting("disable_google_preview") && ($is_google_drive_file || (!is_localhost() && $is_google_preview_available)) ) ) {
        ...
        if ($is_google_drive_file) $src_url = $file_url;
?>
...
<?php } else if ($is_image_file) {?>
    <img src="<?php echo $file_url; ?>" />
<?php } else if (($is_viewable_video_file && !$is_google_drive_file) || $is_google_preview_available) {?>
...
<?php
        ...
        // echo '<br>Google - '.$is_google_preview_available;
        // echo '<br>Video - '.$is_viewable_video_file;
        // echo '<br>Google Drive - '.$is_google_drive_file;
    }
?>
