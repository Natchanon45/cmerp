      ...
      <div class="form-group" style="margin-bottom:0;">
        ...
        <div class="col-lg-2 col-md-3 col-sm-4 col-xs-8">
          <select name="type" class="form-control" required>
            <option value="0" <?php if($item_info->type==0) echo 'selected'; ?>>เป็นวัตถุดิบสำเร็จ</option>
            <option value="1" <?php if($item_info->type==1) echo 'selected'; ?>>เป็นวัตถุดิบผสม</option>
          </select>
        </div>
      </div>

      <div id="type-container" <?php if($item_info->type==0) echo 'style="display:none;"'; ?>>
          ...
          <div class="col-lg-2 col-md-3 col-sm-4 col-xs-8">
            <div class="input-suffix">
              <input 
                ...
                value="<?= sizeof($item_mixing)? $item__mixing[0]->ratio: 0 ?>" 
              />
              <div class="input-tag"><?= $item_info->unit_type ?></div>
            </div>
          </div>
        </div>
        <table class="display dataTable no-footer" cellspacing="0" width="100%" role="grid" aria-describedby="supplier-table_info">            
          ...
          <tbody id="table-body">
            <?php foreach($item_mixing as $k){?>
              <tr>
                <td>
                  <select name="using_item__id[]" class="form-control select-material" required>
                    <option value="" data-unit=""></option>
                    <?php
                      foreach($item_dropdown as $d){
                        $selected = '';
                        if($d->id == $k->using_item_id) $selected = 'selected';
                ...
                <td>
                  <div class="input-suffix">
                    ...
                    />
                    <div class="input-tag"><?= $k->using_item_unit ?></div>
                  </div>
                </td>
                <td class="w100">
