...
if (get_setting("module_order")) {
    ...
    if ($this->login_user->user_type === "staff" && ($this->login_user->is_admin || get_array_value($permissions, "order"))) {
        $can_order = true;
    } else if ($this->login_user->user_type === "client" && get_setting("client_can_access_store")) {
        $can_order = true;
    }
}
...
?>
    <div id="js-init-cart-icon" class="init-chat-icon init-cart-icon">
        ...
        <span id="js-cart-min-icon" data-type="open" class="chat-min-icon"><i class='fa fa-shopping-basket font-18'></i></span>
    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var cartIconContent = {
                "open": "<i class='fa fa-shopping-basket font-18'></i>",
                "close": "<span class='chat-close'>&times;</span>",
                "has_item": ""
            };

            setCartIcon = function (type, count) {
                $("#js-cart-min-icon").attr("data-type", type).html(count ? "<i class='fa fa-shopping-basket font-18'></i><span class='badge bg-danger up cart-badge'>" + count + "</span>" : cartIconContent[type]);
            ...
            $("body").on("click", ".cart-item-quantity-btn", function () {
                ...
                if (action === "minus" && $itemRow.find(".cart-item-quantity").attr("data-quantity") === "1") {
                    ...
                    $.ajax({
                        url: "<?php echo get_uri('pr_items/change_cart_item_quantity'); ?>",
                        cache: false,
                        type: 'POST',
        ...
        function deleteCartItem($itemRow, itemId) {
            if (itemId) {
                $.ajax({
                    url: "<?php echo get_uri('pr_items/delete_cart_item'); ?>",
                    cache: false,
                    type: 'POST',
                    data: {id: itemId},
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            $itemRow.fadeOut(300, function () {
                                ...
                                $gridItem.text("<?php echo lang("add_to_cart"); ?>");
        ...
        function loadCartTabs() {
            setCartIcon("close"); //show close icon
            appLoader.show({container: "#js-rise-cart-wrapper", css: "bottom: 40%; right: 35%;"});

            $.ajax({
                url: "<?php echo get_uri("pr_items/load_cart_items/".@$pr_id); ?>",
                success: function (response) {
        ...
        window.countCartItems = function () {
            $.ajax({
                url: "<?php echo get_uri('pr_items/count_cart_items/'.@$pr_id); ?>",
                cache: false,
                type: 'POST',
                dataType: 'json',
                success: function (response) {
