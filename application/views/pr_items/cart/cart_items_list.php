<?php
$pr_id = @$pr_id?$pr_id:@$_REQUEST['pr_id'];
    ...
    <div class="rise-cart-body">
        ...
        foreach ($items as $item) {
            ...
            <div class='js-item-row pull-left message-row cart-item-row clearfix w100p' data-id='<?php echo $item->id; ?>' data-original_item_id="<?php echo $item->item_id; ?>">
                <?php $this->load->view("pr_items/cart/cart_item_data", array("item" => $item)); ?>
            </div>
        ...
        <div id="cart-total-section" class="cart-item-row pull-left message-row clearfix w100p cart-total no-border mt5 mb5">
            <?php $this->load->view("pr_items/cart/cart_total_section"); ?>
        </div>
    </div>

    <div class="p10 b-t">
        <?php echo anchor(get_uri("purchaserequests/process_pr/".@$pr_id), "<i class='fa fa-check-circle-o'></i> " . lang("process_order"), array("class" => "btn btn-info col-md-12 col-xs-12 col-sm-12")); ?>
    </div>
    <?php
} else {
    ?>
