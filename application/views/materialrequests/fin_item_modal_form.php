...
<script type="text/javascript">
    ...
    function applySelect2OnMTRItemTitle() {
        $("#mr_item_mtr_title").select2({
            showSearchBox: true,
            ajax: {
                url: "<?php echo get_uri("materialrequests/get_item_suggestion"); ?>",
                dataType: 'json',
                quietMillis: 250,
                data: function (term, page) {
                    return {
            ...
            //get existing item info
            $.ajax({
                url: "<?php echo get_uri("materialrequests/get_item_info_suggestion"); ?>",
                ...
                success: function (response) {
                    ...
                    if (response && response.success) {
                        ...
                        $("#item_id").val(response.item_info.id);
                        $("#mr_item_mtr_title").val(response.item_info.title);
                        $("#mr_item_description").val(response.item_info.description);
                        $("#mr_unit_type").val(response.item_info.unit_type);
