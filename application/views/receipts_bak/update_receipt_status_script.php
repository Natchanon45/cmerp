...
<?php } ?>
        ...
        $('body').on('click', '[data-act=update-receipt-status]', function () {
            var $instance = $(this);

            $(this).appModifier({
                value: $(this).attr('data-value'),
                actionUrl: '<?php echo_uri("receipts/save_receipt_status") ?>/' + $(this).attr('data-id'),
                placement: detailsView ? "right" : "auto",
                select2Option: {data: <?php echo json_encode($statuses) ?>},
                onSuccess: function (response, newValue) {
                    if (response.success) {
                        if (detailsView) {
                            $instance.css("background-color", response.receipt_status_color);
                        } else {
