...
<div class="general-form modal-body clearfix">
    ...
    <div class="form-group">
        ...
        <div class="col-md-9 input-suffix">
            <input type="text" name="payments_amount" id="payments_amount" class="form-control" value="<?php echo round($header_info->remain_amount, 2); ?>" autocomplete="off">
...
const maximum_amount = '<?php echo round($header_info->remain_amount, 2); ?>';
...
$(document).ready(function () {
    ...
    $("#btnSubmit").on('click', function (event) {
        ...
        if ($("#payments_amount").val() == '' || pay <= 0) {
            alertController('<?php echo lang("pv_incorrect_amt"); ?>');
            return;
        }

        if (pay > max) {
            alertController('<?php echo lang("pv_pay_amt_exceed_total"); ?>');
            return;
        }

        if ($("#payments_method").val() == '') {
            alertController('<?php echo lang("pv_no_select_pay_method"); ?>');
            return;
        }
        ...
        if ($("#payments_description").val() == '') {
            alertController('<?php echo lang("pv_no_specify_description"); ?>');
            return;
        }
        
        let url = '<?php echo get_uri('payment_voucher/payments_save'); ?>';
        let request = {
            documentId: '<?php echo $header_info->id; ?>',
