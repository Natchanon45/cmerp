<?php
$form_action = isset($contact_user_id) ? get_uri("pay_receipt_taxinvoice/get_stripe_payment_intent_session") : get_uri("receipt_taxinvoice_payments/get_stripe_payment_intent_session");
echo form_open("", array("id" => "stripe-checkout-form", "class" => "pull-left", "role" => "form"));
?>
...
<input type="hidden" name="client_id" value="<?php echo $receipt_taxinvoice_info->client_id; ?>" />
<input type="hidden" name="payment_method_id" value="<?php echo get_array_value($payment_method, "id"); ?>" />
<input type="hidden" name="description" value="<?php echo lang("pay_receipt_taxinvoice"); ?>: (<?php echo to_currency($balance_due, $currency . " "); ?>)" id="description" />

<button type="button" id="stripe-payment-button" class="btn btn-primary mr15"><?php echo get_array_value($payment_method, "pay_button_text"); ?></button>
<?php echo form_close(); ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        var currency = "<?php echo $currency . ' '; ?>",
                payInvoiceText = "<?php echo lang("pay_receipt_taxinvoice"); ?>";
