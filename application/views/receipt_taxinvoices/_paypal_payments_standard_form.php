...
<?php $return_url = isset($verification_code) ? get_uri("pay_receipt_taxinvoice/index/$verification_code") : get_uri("receipt_taxinvoices/preview/$receipt_taxinvoice_id"); ?>
...
<input name="custom" value="receipt_taxinvoice_id:<?php echo $receipt_taxinvoice_info->id; ?>;contact_user_id:<?php echo isset($contact_user_id) ? $contact_user_id : $this->login_user->id; ?>;client_id:<?php echo $receipt_taxinvoice_info->client_id; ?>;payment_method_id:<?php echo get_array_value($payment_method, "id"); ?>" type="hidden"/>
<input name="item_name" value="<?php echo get_receipt_taxinvoice_id($receipt_taxinvoice_info->id); ?>" type="hidden"/>
<input name="item_number" value="<?php echo $receipt_taxinvoice_info->id; ?>" type="hidden"/>

<button id="paypal-payments-stundard-button" class="btn btn-primary mr15"><?php echo get_array_value($payment_method, "pay_button_text"); ?></button>
<?php echo form_close(); ?>
