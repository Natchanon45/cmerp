<?php echo form_open($paytm_url, array("id" => "paytm-online-payment-form", "class" => "pull-left", "role" => "form")); ?>

<input type="hidden" name="MID" value="<?php echo get_array_value($payment_method, "merchant_id"); ?>" />
<input type="hidden" name="ORDER_ID" value="<?php echo "INVOICE-" . $receipt_taxinvoice_id . "-" . rand(10000, 99999999); ?>" />
<input type="hidden" name="CUST_ID" value="<?php echo "CLIENT-" . $receipt_taxinvoice_info->client_id . "-" . rand(10000, 99999999); ?>" />
...
<button type="button" id="paytm-online-payment-button" class="btn btn-primary"><?php echo get_array_value($payment_method, "pay_button_text"); ?></button>
<?php echo form_close(); ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#paytm-online-payment-button").click(function () {
            ...
            //show an error message if user attempt to pay more than the receipt_taxinvoice due and exit
<?php if (get_setting("allow_partial_receipt_taxinvoice_payment_from_clients")) { ?>
                if (unformatCurrency($("#payment-amount").val()) > "<?php echo $balance_due; ?>") {
                    appAlert.error("<?php echo lang("receipt_taxinvoice_over_payment_error_message"); ?>");
                    return false;
                }
<?php } ?>
...
<?php $checksum_hash_url = isset($contact_user_id) ? 'pay_receipt_taxinvoice/get_paytm_checksum_hash' : 'receipt_taxinvoice_payments/get_paytm_checksum_hash'; ?>

            var verificationDataArray = {
                receipt_taxinvoice_id: "<?php echo $receipt_taxinvoice_info->id; ?>",
                contact_user_id: "<?php echo isset($contact_user_id) ? $contact_user_id : $this->login_user->id; ?>",
                client_id: "<?php echo $receipt_taxinvoice_info->client_id; ?>",
