   ...
   //prepare calendar event
    ...
    private function _make_project_event($data, $start_date_event = false) {
        ...
        return array(
            ...
            "borderColor" => $color,
            "cycle" => 0,
            "event_type" => $start_date_event ? "project_start_date" : "project_deadline"
        );
    }
    ...
    private function _make_task_event($data, $start_date_event = false) {
        ...
        return array(
            ...
            "cycle" => 0,
            "event_type" => $start_date_event ? "task_start_date" : "task_deadline"
        );
    }
    ...
    function view() {
		...
		if( !empty( $this->getRolePermission['delete_row'] ) ) {
			...
			$buttonTop[] = js_anchor("<i class='fa fa-times-circle-o'></i> " . lang('delete_event'), array("class" => "btn btn-default pull-left", "id" => "delete_event", "data-encrypted_event_id" => $encrypted_event_id));
		}
		
		if( !empty( $this->getRolePermission['edit_row'] ) ) {
			$buttonTop[] = modal_anchor(get_uri("events/modal_form/"), "<i class='fa fa-pencil'></i> " . lang('edit_event'), array("class" => "btn btn-default", "data-post-encrypted_event_id" => $encrypted_event_id, "title" => lang('edit_event')));
...
$view_data['buttonTop'] = NULL;
    ...
    function show_event_in_google_calendar($google_event_id = "") {
        ...
        $event_link ? redirect($event_link) : show_404();
    }
    ...
    function index($encrypted_event_id = "") {
		...
		if( !empty( $this->getRolePermission['admin'] ) ) {
			...
			//$buttonTop[] = modal_anchor(get_uri("labels/modal_form"), "<i class='fa fa-tags'></i> " . lang('manage_labels'), array("class" => "btn btn-default mb0", "title" => lang('manage_labels'), "data-post-type" => "events"));
		}
		...
		//if( !empty( $this->getRolePermission['add_row'] ) ) {
			...
			//$buttonTop[] = modal_anchor(get_uri("events/modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_event'), array("class" => "btn btn-default", "title" => lang('add_event'), "data-post-client_id" => $client));
		
		//}
		
	//	$buttonTop[] = modal_anchor(get_uri("events/modal_form"), "", array("class" => "hide", "id" => "add_event_hidden", "title" => lang('add_event'), "data-post-client_id" => $client)); 
		
		///$buttonTop[] = modal_anchor(get_uri("events/view"), "", array("class" => "hide", "id" => "show_event_hidden", "data-post-client_id" => $client, "data-post-cycle" => "0", "data-post-editable" => "1", "title" => lang('event_details')));
         
		///$buttonTop[] = modal_anchor(get_uri("leaves/application_details"), "", array("class" => "hide", "data-post-id" => "", "id" => "show_leave_hidden"));
        
	//	$buttonTop[] = modal_anchor(get_uri("projects/task_view"), "", array("class" => "hide", "data-post-id" => "", "id" => "show_task_hidden", "data-modal-lg" => "1"));
		...
		$dfffd = ( count( $view_data['calendar_filter_dropdown'] ) == 1 ) ? "hide" : ""; 

		$buttonTop[] = '<div id="calendar-filter-dropdown" class="pull-left '. $dfffd .'"></div>';
	 
 
		if( !empty( $this->getRolePermission['edit_row'] ) ) {
			
			$buttonTop[] = modal_anchor(get_uri("labels/modal_form"), "<i class='fa fa-tags'></i> " . lang('manage_labels'), array("class" => "btn btn-default", "title" => lang('manage_labels'), "data-post-type" => "event"));  
					
			if ( get_setting( "enable_google_calendar_api" ) ) {
				$buttonTop[] = modal_anchor(get_uri("events/google_calendar_settings_modal_form"), "<i class='fa fa-cog'></i> " . lang('google_calendar_settings'), array("class" => "btn btn-default", "title" => lang('google_calendar_settings')));
			}
		}
...
//arr( $list_data_of_events );

//echo count($list_data_of_events);
            foreach ( $list_data_of_events as $data ) {
				...
				$sql = "
					...
					REPLACE INTO events_read ( user_id, event_id ) 
					SELECT 
