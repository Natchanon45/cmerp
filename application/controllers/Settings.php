...
class Settings extends MY_Controller {
    ...
    function pr() {
        $pr_info = $this->PurchaseRequests_model->get_pr_last_id();
        $view_data["last_id"] = $pr_info;

        $this->template->rander("settings/pr", $view_data);
    }

    function save_pr_settings() {
        $settings = array("purchaserequest_logo", "purchaserequest_prefix", "purchaserequest_color", "purchaserequest_footer", "send_purchaserequest_bcc_to", "initial_number_of_the_purchaserequest", "create_new_projects_automatically_when_purchaserequest_gets_accepted");
        $reload_page = false;
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting === "purchaserequest_footer") {
                $value = decode_ajax_post_data($value);
            } else if ($setting === "purchaserequest_logo" && $value) {
            ...
            if ($setting === "purchaserequest_logo" && $_FILES) {
                $purchaserequest_logo_file = get_array_value($_FILES, "purchaserequest_logo_file");
                $purchaserequest_logo_file_name = get_array_value($purchaserequest_logo_file, "tmp_name");
                if ($purchaserequest_logo_file_name) {
                    $value = serialize(move_temp_file("estimate-logo.png", get_setting("system_file_path"), "", $purchaserequest_logo_file_name));
                    $reload_page = true;
                }
            }
            ...
            if ($setting === "purchaserequest_logo" && !$value) {
                $saveable = false;
            }

            if ($saveable) {
                if ($setting === "purchaserequest_logo") {
                    //delete old file
                    delete_app_files(get_setting("system_file_path"), get_system_files_setting_value("purchaserequest_logo"));
                }

                $this->Settings_model->save_setting($setting, $value);
            }

            if ($setting === "initial_number_of_the_purchaserequest") {
                $this->PurchaseRequests_model->save_initial_number_of_po($value);
            }
        }

        echo json_encode(array("success" => true, 'message' => lang('settings_updated'), "reload_page" => $reload_page));
    }
    
    function po() {
        $po_info = $this->PurchaseRequests_model->get_pr_last_id();
        $view_data["last_id"] = $po_info;

        $this->template->rander("settings/po", $view_data);
    }

    function save_po_settings() {
        $settings = array("po_logo", "po_prefix", "po_color", "po_footer", "send_po_bcc_to", "initial_number_of_the_po", "create_new_projects_automatically_when_po_gets_accepted");
        $reload_page = false;
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting === "po_footer") {
                $value = decode_ajax_post_data($value);
            } else if ($setting === "po_logo" && $value) {
                $value = str_replace("~", ":", $value);
                $value = serialize(move_temp_file("estimate-logo.png", get_setting("system_file_path"), "", $value));
            }
            ...
            //process the file which has uploaded using manual file submit
            if ($setting === "po_logo" && $_FILES) {
                $po_logo_file = get_array_value($_FILES, "po_logo_file");
                $po_logo_file_name = get_array_value($po_logo_file, "tmp_name");
                if ($po_logo_file_name) {
                    $value = serialize(move_temp_file("estimate-logo.png", get_setting("system_file_path"), "", $po_logo_file_name));
                    $reload_page = true;
                }
            }
            ...
            if ($saveable) {
                if ($setting === "po_logo") {
                    //delete old file
                    delete_app_files(get_setting("system_file_path"), get_system_files_setting_value("po_logo"));
                }

                $this->Settings_model->save_setting($setting, $value);
            }

            if ($setting === "initial_number_of_the_po") {
                $this->PurchaseRequests_model->save_initial_number_of_po($value);
            }
        }
    ...
    function delivery() {
        $delivery_info = $this->Deliverys_model->get_delivery_last_id();
        $view_data["last_id"] = $delivery_info;

        $this->template->rander("settings/delivery", $view_data);
    }

    function save_delivery_settings() {
        $settings = array("delivery_logo", "delivery_prefix", "delivery_color", "delivery_footer", "send_delivery_bcc_to", "initial_number_of_the_delivery", "create_new_projects_automatically_when_delivery_gets_accepted");
        $reload_page = false;
        ...
        foreach ($settings as $setting) {
            ...
            //process the file which has uploaded using manual file submit
            if ($setting === "delivery_logo" && $_FILES) {
                $delivery_logo_file = get_array_value($_FILES, "delivery_logo_file");
                $delivery_logo_file_name = get_array_value($delivery_logo_file, "tmp_name");
                if ($delivery_logo_file_name) {
                    $value = serialize(move_temp_file("estimate-logo.png", get_setting("system_file_path"), "", $delivery_logo_file_name));
                    $reload_page = true;
                }
            }
            ...
            if ($saveable) {
                if ($setting === "delivery_logo") {
                    //delete old file
                    delete_app_files(get_setting("system_file_path"), get_system_files_setting_value("delivery_logo"));
                }

                $this->Settings_model->save_setting($setting, $value);
            }

            if ($setting === "initial_number_of_the_delivery") {
                $this->Deliverys_model->save_initial_number_of_delivery($value);
            }
        }
    ...
    function rt() {
        $receipt_taxinvoice_info = $this->Receipt_taxinvoices_model->get_rt_last_id();
        $view_data["last_id"] = $receipt_taxinvoice_info;

        $this->template->rander("settings/rt", $view_data);
    }

    function save_receipt_taxinvoice_settings() {
        $settings = array("rt_logo", "rt_prefix", "rt_color", "rt_footer", "send_rt_bcc_to", "initial_number_of_the_rt", "create_new_projects_automatically_when_rt_gets_accepted");
        $reload_page = false;
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting === "rt_footer") {
                $value = decode_ajax_post_data($value);
            } else if ($setting === "rt_logo" && $value) {
                $value = str_replace("~", ":", $value);
                $value = serialize(move_temp_file("estimate-logo.png", get_setting("system_file_path"), "", $value));
            }
            ...
            //process the file which has uploaded using manual file submit
            if ($setting === "rt_logo" && $_FILES) {
                $rt_logo_file = get_array_value($_FILES, "rt_logo_file");
                $rt_logo_file_name = get_array_value($rt_logo_file, "tmp_name");
                if ($rt_logo_file_name) {
                    $value = serialize(move_temp_file("estimate-logo.png", get_setting("system_file_path"), "", $rt_logo_file_name));
                    $reload_page = true;
                }
            }
            ...
            if ($saveable) {
                if ($setting === "rt_logo") {
                    //delete old file
                    delete_app_files(get_setting("system_file_path"), get_system_files_setting_value("rt_logo"));
                }

                $this->Settings_model->save_setting($setting, $value);
            }

            if ($setting === "initial_number_of_the_rt") {
                $this->Receipt_taxinvoices_model->save_initial_number_of_rt($value);
            }
        }
    ...
    function save_company_settings() {
        $settings = array("company_name", "company_vat_registered", "company_address", "company_phone", "company_email", "company_website", "company_vat_number");
    ...
    function payment_vouchers() {
        $last_invoice_id = $this->Payment_vouchers_model->get_last_invoice_id();

        $view_data["last_id"] = $last_invoice_id;

        $this->template->rander("settings/payment_vouchers", $view_data);
    }

    function save_payment_voucher_settings() {
        $settings = array("allow_partial_invoice_payment_from_clients", "invoice_color", "invoice_footer", "send_bcc_to", "payment_voucher_prefix", "invoice_style", "invoice_logo", "send_invoice_due_pre_reminder", "send_invoice_due_after_reminder", "send_recurring_invoice_reminder_before_creation", "default_due_date_after_billing_date", "initial_number_of_the_invoice", "client_can_pay_invoice_without_login");
        $reload_page = false;
    ...
    function save_module_settings() {
        ...
        /* $settings = array(
            ...
            "module_purchaserequest","module_purchaseorder","module_payment_voucher",
            "module_stock"
        ); */

        $settings = $this->Settings_model->get_module_settings();
    ...
    function orders() {
        ...
        $this->template->rander("settings/orders", $view_data);
    }
