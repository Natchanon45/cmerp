...
class Leads extends MY_Controller
{
	...
	private function _make_row($data, $custom_fields)
	{
		...
		$row_data[] = modal_anchor(get_uri("leads/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_lead'), "data-post-id" => $data->id)) . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_lead'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("leads/delete"), "data-action" => "delete-confirmation"));
	...
	private function _make_file_row($data)
	{
		...
		$description = "<div class='pull-left'>" . js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("leads/view_file/" . $data->id)));
	...
	function validate_import_leads_file_data($check_on_submit = false)
	{
		...
		echo json_encode(
			array(
				...
				'got_error' => ($got_error_header || $got_error_table_data) ? true : false,
	...
	public function leads_index()
	{
		...
		foreach ($custom_fields as $field) {
			...
			LEFT JOIN(
				...
				WHERE `custom_field_values`.`related_to_type` = 'leads' AND `custom_field_values`.`deleted` = 0 AND `custom_field_values`.`custom_field_id` = " . $field->id . "
			) AS `cfv" . $field->id . "` ON `cfv" . $field->id . "`.`related_to_id` = `clients`.`id`
			";
		}
		...
		foreach ($data as $row) {
			$row_data[$index] = array(
				$row->client_name ? anchor(get_uri("leads/view/" . $row->client_id), $row->client_name) : anchor(get_uri("leads/view/" . $row->client_id), "N/A"),
