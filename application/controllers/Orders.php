...
class Orders extends MY_Controller {
    ...
    function process_order() {
        ...
        $view_data["cart_items_count"] = $this->Order_items_model->get_all_where(array("created_by" => $this->login_user->id, "order_id" => 0, "deleted" => 0))->num_rows();
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("orders/item_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_item'), "data-post-id" => $data->id, "data-post-order_id" => $data->order_id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("orders/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function save_item() {
        ...
        if ($order_item_id) {
            ...
            echo json_encode(array("success" => true, "order_id" => $item_info->order_id, "data" => $this->_make_item_row($item_info), "order_total_view" => $this->_get_order_total_view($item_info->order_id), 'id' => $order_item_id, 'message' => lang('record_saved')));
        } else {
    ...
    function delete_item() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Order_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Order_items_model->get_details($options)->row();
                echo json_encode(array("success" => true, "order_id" => $item_info->order_id, "data" => $this->_make_item_row($item_info), "order_total_view" => $this->_get_order_total_view($item_info->order_id), "message" => lang('record_undone')));
            } else {
            ...
            if ($this->Order_items_model->delete($id)) {
                $item_info = $this->Order_items_model->get_one($id);
                echo json_encode(array("success" => true, "order_id" => $item_info->order_id, "order_total_view" => $this->_get_order_total_view($item_info->order_id), 'message' => lang('record_deleted')));
            } else {
    ...
    function place_order() {
        ...
        $order_items = $this->Order_items_model->get_all_where(array("created_by" => $this->login_user->id, "order_id" => 0, "deleted" => 0))->result();
        if (!$order_items) {
            show_404();
        }
        ...
        $order_data = array(
            ...
            "tax_id" => $this->input->post('order_tax_id') ? $this->input->post('order_tax_id') : 0,
            "tax_id2" => $this->input->post('order_tax_id2') ? $this->input->post('order_tax_id2') : 0
            /* "tax_id" => get_setting('order_tax_id') ? get_setting('order_tax_id') : 0,
            "tax_id2" => get_setting('order_tax_id2') ? get_setting('order_tax_id2') : 0 */
        );

        $sqlname = "SELECT company_name FROM clients WHERE id = " . $this->input->post("client_id") . "";
        // arr($sqlname);exit;
        $ss = $this->fetched_data = $this->db->query($sqlname)->row();

        // var_dump($ss->company_name);exit;

        // var_dump($ss,$order_items);exit;
        
        
        // var_dump($str,$tol);exit;
        ...
        if ($order_id) {
            ...
            $str = '';
            $str .= $ss->company_name . "\n";
            $str .= 'ได้ทำการสั่งซื้อสินค้า '."\n";
            $tol = 0;
            foreach($order_items as $order) {
                $str .= $order->title . "\n";
                $str .= 'จำนวน : ' .intval($order->quantity). "\n";
                $str .= 'ราคา : ' .intval($order->rate). "\n";
                $tol = $order->total + $tol;
            }
            $str .= 'ราคารวม : ' .$tol. ' บาท'. "\n";
            $str .= 'ดูใบสั่งซื้อ : '.$redirect_to.'';
    ...
    private function _make_row($data, $custom_fields) {
        $order_url = "";
        if ($this->login_user->user_type == "staff") {
            $order_url = anchor(get_uri("orders/view/" . $data->id), $data->doc_no);
        } else {
            //for client
            $order_url = anchor(get_uri("orders/preview/" . $data->id), $data->doc_no);
        }
        ...
        if ($this->login_user->user_type == "staff") {
            $row_data[] = js_anchor($data->order_status_title, array("style" => "background-color: $data->order_status_color", "class" => "label", "data-id" => $data->id, "data-value" => $data->status_id, "data-act" => "update-order-status"));
        } else {
        ...
        $row_data[] = modal_anchor(get_uri("orders/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_order'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_order'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("orders/delete"), "data-action" => "delete"));
    ...
    function save_order_status($id = 0) {
        ...
        if ($save_id) {
            log_notification("order_status_updated", array("order_id" => $id));
            $order_info = $this->Orders_model->get_details(array("id" => $id))->row();
            echo json_encode(array("success" => true, "data" => $this->_row_data($save_id), 'id' => $save_id, "message" => lang('record_saved'), "order_status_color" => $order_info->order_status_color));
        } else {
            echo json_encode(array("success" => false, lang('error_occurred')));
        }
    }
    ...
    function save_discount() {
        ...
        if ($save_data) {
            echo json_encode(array("success" => true, "order_total_view" => $this->_get_order_total_view($order_id), 'message' => lang('record_saved'), "order_id" => $order_id));
        } else {
            echo json_encode(array("success" => false, 'message' => lang('error_occurred')));
        }
    }
    ...
    function item_list_data($order_id = 0) {
        ...
        $list_data = $this->Order_items_model->get_details(array("order_id" => $order_id))->result();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_item_row($data);
        }
