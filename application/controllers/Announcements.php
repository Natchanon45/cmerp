...
class Announcements extends MY_Controller {
    ...
    function save() {
        ...
        if ($save_id) {
            ...
            echo json_encode(array("success" => true, "recirect_to" => get_uri("announcements/form/" . $save_id), 'id' => $save_id, 'message' => lang('record_saved')));
        } else {
    ...
    function list_data() {
        ...
        $list_data = $this->Announcements_model->get_details( $options, $this->getRolePermission )->result();
        ...
        foreach ( $list_data as $data ) {
			...
			if( !empty( $this->getRolePermission['edit_row'] ) ){
				$optionss[] = anchor( 
					get_uri( "announcements/form/" . $data->id ), 
					"<i class='fa fa-pencil'></i>", 
					array( "class" => "edit", "title" => lang( 'edit_announcement' ) ) 
			...
			if( !empty( $this->getRolePermission['delete_row'] ) ){
				$optionss[] = js_anchor( "<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_announcement'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("announcements/delete"), "data-action" => "delete"));
			}
