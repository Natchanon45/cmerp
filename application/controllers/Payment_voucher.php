...
class Payment_voucher extends MY_Controller {
    ...
    function index() {
        ...
        if($this->input->post("datatable") == true){
            ...
            if($this->json->task == "update_doc_status") jout($this->Payment_voucher_m->updateStatus());
            return;    
        }
    ...
    function addedit(){
        if(isset($this->json->task)){
            if($this->json->task == "save_doc") jout($this->Payment_voucher_m->saveDoc());
            return;   
        }
    ...
    function view() {
        if(isset($this->json->task)){
            if($this->json->task == "load_items") jout($this->Payment_voucher_m->items());
            if($this->json->task == "update_doc") jout($this->Payment_voucher_m->updateDoc());
            if($this->json->task == "delete_item") jout($this->Payment_voucher_m->deleteItem());
            return;
        }
    ...
    function item() {
        if(isset($this->json->task)){
            if($this->json->task == "save") jout($this->Payment_voucher_m->saveItem());
            return;   
        }
