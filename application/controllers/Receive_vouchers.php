...
class Receive_vouchers extends MY_Controller {
    ...
    function yearly() {
        $this->load->view("receive_vouchers/yearly_invoices");
    }
    ...
    function recurring() {
        ...
        $this->load->view("receive_vouchers/recurring_invoices_list", $view_data);
    }

    //load the custom view of invoice list 
    function custom() {
        $this->load->view("receive_vouchers/custom_invoices_list");
    }
	...
	private function _copy_estimate_or_order_items_to_invoice($copy_items_from_estimate, $copy_items_from_order, $invoice_id) {
        ...
        foreach ($items as $data) {
            ...
            $this->Payment_voucher_items_model->save($invoice_item_data);
        }
    }
    ...
    function delete() {
        ...
        $invoice_info = $this->Receive_vouchers_model->get_one($id);

        if ($this->Receive_vouchers_model->delete($id)) {
            //delete the files
    ...
    function invoice_list_data_of_client($client_id) {
        ...
        $list_data = $this->Receive_vouchers_model->get_details($options)->result();
        $result = array();
    ...
    function sub_invoices($recurring_invoice_id) {
        ...
        $this->load->view("receive_vouchers/sub_invoices", $view_data);
    }
    ...
    private function _row_data($id) {
        ...
        $data = $this->Receive_vouchers_model->get_details($options)->row();
    ...
    private function _make_recurring_row($data) {
        ...
        $invoice_url = anchor(get_uri("receive_vouchers/view/" . $data->id), get_payment_voucher_id($data->id));
    ...
    private function _get_invoice_total_view($invoice_id = 0) {
        $view_data["invoice_total_summary"] = $this->Receive_vouchers_model->get_invoice_total_summary($invoice_id);
        $view_data["invoice_id"] = $invoice_id;
        $view_data["can_edit_invoices"] = $this->can_edit_invoices();
        return $this->load->view("receive_vouchers/invoice_total_section", $view_data, true);
    }
    ...
    function item_modal_form() {
        ...
        $view_data['invoice_id'] = $invoice_id;
        $this->load->view("receive_vouchers/item_modal_form", $view_data);
    }
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("receive_vouchers/item_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_payment_voucher'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("receive_vouchers/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function preview($invoice_id = 0, $show_close_preview = false) {
        if ($invoice_id) {
            ...
            $this->template->rander("receive_vouchers/invoice_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function print_invoice($invoice_id = 0) {
        if ($invoice_id) {
            ...
            echo json_encode(array("success" => true, "print_view" => $this->load->view("receive_vouchers/print_invoice", $view_data, true)));
        } else {
            echo json_encode(array("success" => false, lang('error_occurred')));
        }
    }
    ...
    function send_invoice_modal_form($invoice_id) {
        ...
        if ($invoice_id) {
            ...
            $invoice_info = $this->Receive_vouchers_model->get_details($options)->row();
            ...
            $this->load->view("receive_vouchers/send_invoice_modal_form", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function get_send_invoice_template($invoice_id = 0, $contact_id = 0, $return_type = "", $invoice_info = "", $contact_info = "") {
        ...
        $invoice_total_summary = $this->Receive_vouchers_model->get_invoice_total_summary($invoice_id);
        ...
        $parser_data["INVOICE_URL"] = get_uri("receive_vouchers/preview/" . $invoice_info->id);
    ...
    function send_invoice() {
        ...
        if (send_app_mail($contact->email, $subject, $message, array("attachments" => $attachments, "cc" => $cc_array, "bcc" => $bcc_emails))) {
            ...
            if ($this->Receive_vouchers_model->save($status_data, $invoice_id)) {
    ...
    function get_invoice_status_bar($invoice_id = 0) {
        ...
        $view_data["invoice_info"] = $this->Receive_vouchers_model->get_details(array("id" => $invoice_id))->row();
        $view_data['invoice_status_label'] = $this->_get_invoice_status_label($view_data["invoice_info"]);
        $this->load->view("receive_vouchers/invoice_status_bar", $view_data);
    }
    ...
    function update_invoice_status($invoice_id = 0, $status = "") {
        ...
        if ($invoice_id && $status) {
            ...
            $this->Receive_vouchers_model->update_invoice_status($invoice_id, $status);
            ...
            if ($status == "cancelled") {
                ...
                $this->Receive_vouchers_model->save($data, $invoice_id);
            }
    ...
    function discount_modal_form() {
        ...
        $view_data['model_info'] = $this->Receive_vouchers_model->get_one($invoice_id);

        $this->load->view("receive_vouchers/discount_modal_form", $view_data);
    }
    ...
    function save_discount() {
        ...
        $save_data = $this->Receive_vouchers_model->save($data, $invoice_id);
        if ($save_data) {
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            $this->load->view("receive_vouchers/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function modal_form() {
        ...
        $model_info = $this->Receive_vouchers_model->get_one($this->input->post('id'));
        ...
        $this->load->view("receive_vouchers/modal_form", $view_data);
    }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($this->login_user->user_type == "staff") {
           ...
           /// $invoice_url = anchor(get_uri("receive_vouchers/preview/" . $data->id), get_payment_voucher_id($data->id));
        }
    ...
    private function _make_options_dropdown( $invoice_id = 0 ) {
		...
		///if( !empty( $this->getRolePermission['edit_row'] ) ) {
			$buttons[] = '<li role="presentation">' . modal_anchor(get_uri("receive_vouchers/modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), array("title" => lang('edit_payment_voucher'), "data-post-id" => $invoice_id)) . '</li>';
		//}
		
		//if( !empty( $this->getRolePermission['delete_row'] ) ) {
			$buttons[] = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), array('title' => lang('delete_payment_voucher'), "class" => "delete", "data-id" => $invoice_id, "data-action-url" => get_uri("receive_vouchers/delete"), "data-action" => "delete-confirmation")) . '</li>';
		//}
    ...
    function view( $invoice_id = 0 ) {
        ...
        if ( $invoice_id ) {
			...
			$invoice_info = $this->Receive_vouchers_model->get_details( array( "id" => $invoice_id ) )->row();
			if ( $invoice_info ) {
				$data['invoice_info'] = $invoice_info;
				$data['client_info'] = $this->Clients_model->get_one($data['invoice_info']->client_id);
				$data['invoice_items'] = $this->Payment_voucher_items_model->get_details(array("invoice_id" => $invoice_id))->result();
				$data['invoice_status_label'] = get_invoice_status_label($invoice_info);
				//$data["invoice_total_summary"] = $this->Receive_vouchers_model->get_invoice_total_summary($invoice_id);
                ...
                $this->template->rander("receive_vouchers/view", $view_data);
			}
			else {
                show_404();
            }
    ...
    function save() {
        ...
        $invoice_id = $this->Receive_vouchers_model->save($invoice_data, $id);
        ...
        if ($invoice_id) {
            ...
            if ($is_clone && $main_invoice_id) {
                ...
                $invoice_items = $this->Payment_voucher_items_model->get_all_where(array("invoice_id" => $main_invoice_id, "deleted" => 0))->result();
                ...
                foreach ($invoice_items as $invoice_item) {
                    ...
                    $invoice_item = $this->Payment_voucher_items_model->save($invoice_item_data);
                }
            } else {
    ...
    function list_data() {
        ...
        $custom_fields = $this->Custom_fields_model->get_available_fields_for_table( "receive_vouchers", $this->login_user->is_admin, $this->login_user->user_type );
	...
	//	exit;
        ...
        $list_data = $this->Receive_vouchers_model->get_details( $options, $this->getRolePermission )->result();
        $result = array();
        foreach ($list_data as $data ) {
...
//exit;
		...
		*/
		...
		///if( !empty( $this->getRolePermission['add_row'] ) ) {
			...
			$buttonTop[] = modal_anchor(get_uri("receive_vouchers/modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_payment_voucher'), array("class" => "btn btn-default mb0", "title" => lang('add_payment_voucher')));
		...
		$this->template->rander("receive_vouchers/index", $view_data );
