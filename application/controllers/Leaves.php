...
class Leaves extends MY_Controller {
    ...
    private function _prepare_leave_info($data) {
        ...
        if(isset($data->created_at)){
            $data->created_date = format_to_date($data->created_at, FALSE);
        }
        ...
        if (isset($data->start_date)) {
            ...
            if ($data->start_date != $data->end_date) {
                ...
                $date = format_to_date($data->start_date, FALSE) ." - ".format_to_date($data->end_date, FALSE);
            }
            $data->date_meta = $date;
        }
        if ($data->total_days > 1) {
            $duration = (int)$data->total_days . " " . lang("days");
        } else {
            $duration = (int)$data->total_days . " " . lang("day");
        }
