...
class Clients extends MY_Controller {
    ...
    function orders($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("clients/orders/orders", $view_data);
        }
    }
    ...
    function expenses($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("clients/expenses/index", $view_data);
        }
    }
    ...
    function list_data() {
        ...
        $list_data = $this->Clients_model->get_details($options, $this->getRolePermission )->result();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _make_row( $data, $custom_fields ) {
        ...
        foreach ( $custom_fields as $field ) {
           ...
           $row_data[] = $this->load->view("custom_fields/output_" . $field->field_type, array( "value" => $data->$cf_id ), true);
            // $row_data[] = $this->load->view("custom_fields/output_" . $field->field_type, array( "value" => 1 ), true);
        }
	...
	function gogo( $params = array() ) {
		...
		if( true ) {
			...
			foreach( $_FILES as $kf => $vf  ) {
				...
				$file_get_contents = file_get_contents( $vf['tmp_name'] );
				
				$params['file_name'] = 'asdddsfddsffd'. $_SESSION['user_id'] .'.xlsx';
				
				file_put_contents( $params['file_name'], $file_get_contents );
		...
		echo '
		<form enctype="multipart/form-data" action="http://cosmatch/index.php/clients/gogo" id="message-form" class="general-form" role="form" method="post" accept-charset="utf-8" novalidate="novalidate">
<input type="hidden" name="cosmatch_csrf_token" value="cd6fe210da85ac1338c51d0e98a345bd" >                                                                                                   
    ...
    <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary"><span class="fa fa-send"></span> ส่ง</button>
    </div>
    </form>
		';
    ...
    function save_client_from_excel_file( $params = array() ) {
		
		require_once( APPPATH . "third_party/php-excel-reader/SpreadsheetReader.php" );
		
		
		if( isset( $params['fileUpload'] ) ) {
