...
class Messages extends MY_Controller {
	...
	//echo $this->load->view( "messages/view", $view_data, true );
	...
	// arr( $_REQUEST );
		$ors = array();
		if( !empty( $request['userCheckboxs'] ) ) {
			
			$ors[] = "id IN ( ". implode( ',', $request['userCheckboxs'] ) ." )";
		}
		
		if( !empty( $request['roles'] ) ) {
			
			$ors[] = "`role_id` IN ( ". implode( ',', $request['roles'] ) ." )";
		}
		
		if( !empty( $request['team'] ) ) {
			
			$sql = "
				SELECT * FROM `team` WHERE `id` IN ( ". implode( ',', $request['team'] ) ." )
			";
			
			foreach( $this->dao->fetchAll( $sql ) as $kt => $vt )  {
				//arr( $vt );
				$ors[] = "`id` IN ( ". $vt->members ." )";
			}
		...
		$param['filters']['WHERE'][] = "( ". implode( ' or ', $ors ) ." )";
		...
		foreach( $this->dao->fetchAll( $sql ) as $km => $vm ) {
			...
			if( !empty( $request['note_id'] ) ) {
				$message_data["note_id"] = $request['note_id']; 
				
				
				$sql = "
					SELECT * FROM `notes` WHERE `id` = ". $request['note_id'] ."
					
				";
				
				///arr( $sql );
				///foreach( $this->dao->fetchAll( $sql ) as $kn => $vn ) {
					///$message_data["files"] = $vn->files; 
				///}
...
///http://cosmatch/index.php/messages/download_message_files/4
	...
	//arr( $model_info);
		...
		$sql = "
			SELECT * FROM `notes` WHERE `id` = ". $model_info->note_id ."
		";
		
		$files = array();
		foreach( $this->dao->fetchAll( $sql ) as $ka => $va ) {
			
			foreach( unserialize( $va->files ) as $kf => $vf ) {
				$files[] = $vf;
		...
		foreach( unserialize( $model_info->files ) as $kf => $vf ) {
			$files[] = $vf;
