...
class Stock extends MY_Controller {
  ...
  function suppliers() {
    ...
    if(!$this->bom_can_access_supplier()) redirect("forbidden");
  ...
  function supplier_modal() {
    ...
    if(empty($view_data['model_info']->id)) {
      if(!$this->check_permission('bom_supplier_create')) redirect("forbidden");
    } else {
      $owner_id = $view_data['model_info']->owner_id;
      if(!$this->bom_can_read_supplier($owner_id)) redirect("forbidden");
    }
  ...
  function supplier_view_file($file_id = 0) {
    ...
    if ($file_info) {
      if (!$file_info->supplier_id) redirect("forbidden");
  ...
  function material_modal() {
    ...
    if(empty($view_data['model_info']->id)) {
      if(!$this->check_permission('bom_material_create')) redirect("forbidden");
    }
  ...
  function material_view_file($file_id = 0) {
    ...
    if ($file_info) {
      if (!$file_info->material_id) redirect("forbidden");
  ...
  function restocks(){
    $this->check_module_availability("module_stock");
    if(!$this->bom_can_access_restock()) redirect("forbidden");
  ...
  function restock_modal() {
    ...
    if(empty($view_data['model_info']->id)) {
      if(!$this->check_permission('bom_restock_create')) redirect("forbidden");
    } else {
      $created_by = $view_data['model_info']->created_by;
      if(!$this->bom_can_read_restock($created_by)) redirect("forbidden");
    }
    ...
    if ($save_id) {
      ...
      if (isset($restock_ids) && isset($material_ids) && isset($stocks)) {
        $this->Bom_stock_groups_model->restock_save(
          $save_id, $restock_ids, $material_ids, $stocks, $prices
        );
      }

      echo json_encode(array(
        "success" => true, 
        "data" => $this->_restock_row_data($save_id), 
        'id' => $save_id, 
        'view' => $this->input->post('view'), 
        'message' => lang('record_saved')
      ));
  ...
  function calculator(){
    $this->check_module_availability("module_stock");
    if(!$this->bom_can_access_calculator()) redirect("forbidden");
    ...
    if ($save_id) {
      ...
      if (isset($restock_ids) && isset($item_ids) && isset($stocks)) {
        $this->Bom_item_groups_model->restock_item_save(
          $save_id, $restock_ids, $item_ids, $stocks, $prices
        );
      }
...
// END: Material Remaining
  ...
  function item_view_file($file_id = 0) {
    ...
    if ($file_info) {
      if (!$file_info->item_id) redirect("forbidden");
