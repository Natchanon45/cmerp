...
class Help extends MY_Controller {
    ...
    function save_article() {
		...
		redirect( get_uri( "help/knowledge_base_articles" ));
    }
    ...
    function articles_list_data( $type ) {
        ...
        $list_data = $this->Help_articles_model->get_details( array( "type" => $type ), $this->getRolePermission )->result();
        $result = array();
        foreach ($list_data as $data) {
    ...
    function article_form( $type, $id = 0 ) {
		...
		foreach( $this->db_model->fetchAll( $sql ) as $ka => $va )  {
			...
			if( isset( $view_data['model_info']->category_id ) && $view_data['model_info']->category_id == $va->id ) {
				$select =  ' selected ';
			}
			$options[] = '<option'. $select .'value="'. $va->id .'">'. $va->title .'</option>';
		...
		$view_data['categories_selet'] = '<select name="category_id" class="select2 validate-hidden">'. implode( '', $options ) .'</select>';
