...
class Team_members extends MY_Controller {
    ...
    function modal_form_signature() {
        ...
        $this->load->view('team_members/files/modal_form_signature', $view_data);
    }
    ...
    private function get_recently_meaning_dropdown() {
        return array(
            "2_hours" => lang("in") . " 2 " . strtolower(lang("hours")),
            "5_hours" => lang("in") . " 5 " . strtolower(lang("hours")),
            "8_hours" => lang("in") . " 8 " . strtolower(lang("hours")),
            "1_days" => lang("in") . " 1 " . strtolower(lang("day")),
            "2_days" => lang("in") . " 2 " . strtolower(lang("days")),
            "3_days" => lang("in") . " 3 " . strtolower(lang("days")),
            "5_days" => lang("in") . " 5 " . strtolower(lang("days")),
            "7_days" => lang("in") . " 7 " . strtolower(lang("days")),
            "15_days" => lang("in") . " 15 " . strtolower(lang("days")),
            "1_month" => lang("in") . " 1 " . strtolower(lang("month")),
        );
    }

    function recently_meaning_modal_form() {
        $view_data["recently_meaning_dropdown"] = $this->get_recently_meaning_dropdown();
        $this->load->view('projects/tasks/recently_meaning_modal_form', $view_data);
    }

    function save_recently_meaning() {
        $recently_meaning = $this->input->post("recently_meaning");
    ...
    function view($id = 0, $tab = "") {
        ...
        if ( intval($id) ) {
		 ...
		 // arr( $this->getRolePermission['p']['can_view_team_members_social_links'] );exit;
//
...
$view_data['tabs'] = implode( '', $tabs );
    ...
    function social_links( $user_id ) {
		...
		$view_data['button'] = ''; 
		if( $this->getSubmitButton( $user_id ) ) {
			$view_data['button'] = '  <div class="panel-footer">
                <button type="submit" class="btn btn-primary"><span class="fa fa-check-circle"></span> '. lang('save') .'</button>
            </div>'; 
	...
	function getSubmitButton( $user_id = NULL ) {
		...
		else if( $this->getRolePermission['p']['team_member_update_permission'] == 'specific' ) {
			
			foreach( explode( ',', $this->getRolePermission['p']['team_member_update_permission_specific'] ) as $ke => $ve ) {
				$ex = explode( ':', $ve );
				//arr( $ex );
				...
				if( $ex[0] == 'team' ) {
					...
					$sql = "
						SELECT  `members`   FROM `team` WHERE `id` = ". $ex[1] ."
					";
					
					
					foreach( $this->dao->fetchAll( $sql ) as $kt => $vt ) {
						
						if( in_array( $user_id, explode( ',', $vt->members ) ) ) {
							return true;
						}
					}
				}
