...
class Receive_voucher_payments extends MY_Controller {
    ...
    function index() {
        if ($this->login_user->user_type === "staff") {
            ...
            $this->template->rander("receive_vouchers/payment_received", $view_data);
        } else {
    ...
    function yearly() {
        $this->load->view("receive_vouchers/yearly_payments");
    }

    //load custom payment list
    function custom() {
        $this->load->view("receive_vouchers/custom_payments_list");
    }
    ...
    function save_payment() {
        ...
        if ($invoice_payment_id) {
            ...
            $this->Receive_vouchers_model->update_invoice_status($invoice_id);
    ...
    private function _make_payment_row($data) {
        ...
        if ($this->login_user->user_type == "staff") {
            $invoice_url = anchor(get_uri("receive_vouchers/view/" . $data->invoice_id), get_payment_voucher_id($data->invoice_id));
        } else {
            $invoice_url = anchor(get_uri("receive_vouchers/preview/" . $data->invoice_id), get_payment_voucher_id($data->invoice_id));
        }
        return array(
            $invoice_url,
            $data->payment_date,
            ...
            modal_anchor(get_uri("payment_voucher_payments/payment_modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_payment'), "data-post-id" => $data->id, "data-post-invoice_id" => $data->invoice_id,))
    ...
    function yearly_chart() {
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown();
        $this->load->view("receive_vouchers/yearly_payments_chart", $view_data);
    }
...
*/
        ...
        $view_data['model_info'] = $this->Payment_voucher_payments_model->get_one($this->input->post('id'));
        ...
        if (!$invoice_id) {
            //prepare payment_vouchers dropdown
        }
		...
		$view_data['invoice_id'] = $invoice_id;
			...
			$view_data['invoice_id_s'] =  ' 
				...
				<label for="invoice_id" class=" col-md-3">'. lang('invoice') .'</label>
				<div class="col-md-9">
				'. form_dropdown("invoice_id", $view_data['invoices_dropdown'], "", "class='select2 validate-hidden' id='invoice_id' data-rule-required='true' data-msg-required='" . lang('field_required') . "' ") .'
				</div>
				</div>
			';
        ...
        $this->load->view( 'receive_vouchers/payment_modal_form', $view_data );
    }
