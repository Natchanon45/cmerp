...
class Google_calendar {
    ...
    private function _check_calendar_ids($client, $user_id = "") {
        ...
        foreach ($calendar_ids_array as $calendar_id) {
            try {
                ...
                $this->ci->session->set_flashdata("error_message", lang('invalid_calendar_id_error_message') . ": " . $calendar_id);
                redirect(get_uri("events"));
            }
        }
    }
        ...
        if ($user_id && $id && get_setting("enable_google_calendar_api") && get_setting("module_event")) {
            ...
            if ($event_info) {
                ...
                if ($calendar_event_info) {
                    ...
                    $this->ci->Events_model->save($data, $id);
                }
            }
        }
    }
    ...
    private function _get_share_with_emails($event_info = "") {
        if ($event_info && $event_info->share_with) {
            ...
            $users = $this->ci->Events_model->get_share_with_users_of_event($event_info)->result();
    ...
    public function get_google_calendar_events() {
        if (get_setting("enable_google_calendar_api") && get_setting("module_event")) {
            $enabled_users_settings = $this->ci->Events_model->get_integrated_users_with_google_calendar()->result();
            ...
            foreach ($enabled_users_settings as $setting) {
                ...
                if ($user_id && get_setting('user_' . $user_id . '_oauth_access_token') && get_setting('user_' . $user_id . '_google_calendar_authorized')) {
                    ...
                    $user_info = $this->ci->Users_model->get_details($user_options)->row();
                    if (!$user_info->id) {
                        continue;
                    }
    ...
    private function _prepare_calendar_events($events = array(), $user_id = 0) {
        ...
        $label_data = array("title" => lang("google_calendar_event"), "color" => "#2d9cdb", "context" => "event", "user_id" => $user_id);
        ...
        if ($existing_label->id) {
            ...
            $label_id = $this->ci->Labels_model->save($label_data);
        }
        ...
        foreach ($events as $event) {
            ...
            //if the event is deleted from Google, delete from the platform too
    ...
    private function _save_calendar_events($data = array(), $google_event_id = "") {
        ...
        $this->ci->Events_model->save($data, $existing_event->id);
    }
