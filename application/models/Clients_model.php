...
class Clients_model extends Crud_model
{
	...
	function get_details($options = array(), $getRolePermission = array())
	{
		...
		$sql = "SELECT $clients_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS primary_contact, $users_table.id AS primary_contact_id, $users_table.image AS contact_avatar,  project_table.total_projects, $payment_value_select AS payment_received $select_custom_fieds,
                ...
                (SELECT GROUP_CONCAT($client_groups_table.title) FROM $client_groups_table WHERE FIND_IN_SET($client_groups_table.id, $clients_table.group_ids)) AS client_groups, $lead_status_table.title AS lead_status_title,  $lead_status_table.color AS lead_status_color, owner_details.owner_name, owner_details.owner_avatar 
		FROM $clients_table 
	...
	function get_primary_contact($client_id = 0, $info = false)
	{
		...
		FROM $users_table WHERE $users_table.deleted=0 AND $users_table.client_id=$client_id AND $users_table.is_primary_contact=1";
	...
	function add_remove_star($project_id, $user_id, $type = "add")
	{
		...
		$sql = "UPDATE $clients_table SET $clients_table.starred_by = $action WHERE $clients_table.id=$project_id $where";
		return $this->db->query($sql);
	}
	...
	function get_leads_kanban_details($options = array())
	{
		...
		WHERE $clients_table.deleted=0 AND $clients_table.is_lead=1 $where ORDER BY new_sort ASC";
	...
	function get_search_suggestion($search = "", $options = array())
	{
		...
		ORDER BY $clients_table.company_name ASC LIMIT 0, 10";
	...
	function count_total_clients($show_own_clients_only_user_id = "")
	{
		...
		$sql = "SELECT COUNT($clients_table.id) AS total FROM $clients_table 
