...
class Invoices_model extends Crud_model {
	...
	function get_invoice_total_summary($invoice_id = 0, $new_data = array()) {
        ...
        FROM $invoices_table
        LEFT JOIN estimates ON $invoices_table.es_id = estimates.id
        ...
        $result->tax_id = $invoice->tax_id;
        $result->tax_id2 = $invoice->tax_id2;
        $result->tax_id3 = $invoice->tax_id3;

        $result->credit = $invoice->credit;
        $result->total_es = $invoice->total_es;
        $result->sub_total_es = $invoice->sub_total_es;
        $result->pay_es_sps = $invoice->pay_es_sps;
        
        $result->pay_spilter = $invoice->pay_spilter;
        $result->pay_time = $invoice->pay_time;
        $result->deposit = $invoice->deposit;
        $result->include_deposit = $invoice->include_deposit;
        ...
        if($invoice->tax_percentage == 7){
            $result->invoice_total = $invoice_subtotal + $result->tax - $result->tax2;
        }else{
            $result->invoice_total = $invoice_subtotal - $result->tax + $result->tax2;
        }
    ...
    function get_details($options = array(), $getRolePermission = array() ) {
        ...
        $sql = "
			SELECT
                $invoices_table.es_id, 
				$invoices_table.pay_type, 
				$invoices_table.pay_spilter, 
				$invoices_table.id, 
                $invoices_table.doc_no,
				$invoices_table.created_by, 
				$invoices_table.client_id, $invoices_table.project_id, $invoices_table.bill_date, $invoices_table.due_date, $invoices_table.note, $invoices_table.labels, $invoices_table.last_email_sent_date, 
				IF( pt.id IS NULL, $invoices_table.status, 'อนุมัติ' ) status, 
				
				$invoices_table.tax_id, $invoices_table.tax_id2, $invoices_table.tax_id3, $invoices_table.recurring, $invoices_table.recurring_invoice_id, $invoices_table.repeat_every, $invoices_table.repeat_type, $invoices_table.no_of_cycles, $invoices_table.next_recurring_date, $invoices_table.no_of_cycles_completed, $invoices_table.due_reminder_date, $invoices_table.recurring_reminder_date, $invoices_table.discount_amount, $invoices_table.discount_amount_type, $invoices_table.discount_type, $invoices_table.cancelled_at, $invoices_table.cancelled_by, $invoices_table.files, $invoices_table.deleted,			
				$clients_table.currency, 
				$clients_table.currency_symbol, 
				$clients_table.company_name, 
				$projects_table.title AS project_title,
				$invoices_table.pay_spilter AS invoice_value, 
                ...
                ,estimates.doc_no as es_doc_no,
                estimates.deposit as deposit,
                $invoices_table.include_deposit as include_deposit
            FROM $invoices_table 
            LEFT JOIN estimates ON estimates.id = $invoices_table.es_id
			LEFT JOIN prove_table pt ON $invoices_table.id = pt.doc_id AND pt.tbName = '". $invoices_table ."'
			...
			LEFT JOIN ( 
				SELECT 
					$taxes_table.* 
				FROM $taxes_table 
			) AS tax_table3 ON tax_table3.id = $invoices_table.tax_id3
			LEFT JOIN (
				SELECT 
					invoice_id, 
					SUM(amount) AS payment_received 
				FROM $invoice_payments_table 
				WHERE deleted = 0 GROUP BY invoice_id ) AS payments_table ON payments_table.invoice_id = $invoices_table.id 
			LEFT JOIN ( 
				SELECT 
					invoice_id, 
				...
				GROUP BY 
					invoice_id 
			) AS items_table ON items_table.invoice_id = $invoices_table.id 
			$join_custom_fieds
			[WHERE]
		";
		...
		//arr( $options );
		if( !empty( $options['id'] ) ) {
			$filters['WHERE'][] = "". $invoices_table .".id = ". $options['id'] ."";
    ...
    public function getInvoice_Item_suggestion( $keyword = "", $id = null,$datas = NULL ) {
        ...
        if($id || $datas){
          ...
          IFNULL(
					(
						SELECT
							sum( it.quantity )
						FROM receipt_taxinvoice_items it
						INNER JOIN receipt_taxinvoices inv ON it.receipt_taxinvoice_id = inv.id
						[WHERE]

					)
					,0
				) as recetpQty
                FROM invoice_items ei
                LEFT JOIN invoices on invoices.id = ei.invoice_id
                INNER JOIN prove_table ptt ON ptt.tbName = 'invoices' AND ei.invoice_id = ptt.doc_id
                GROUP BY invoice_id
            $where
          ";
//
          ...
          $filters = array();
          $filters['WHERE'][] = "
          it.receipt_taxinvoice_id = ei.invoice_id
          AND it.deleted = 0
          AND inv.deleted = 0
          ";
