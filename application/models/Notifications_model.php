...
class Notifications_model extends Crud_model {
    ...
    function create_notification($event, $user_id, $options = array()) {
        ...
        $role_perm_table = $this->db->dbprefix('role_permission');
        ...
        $pr_table = $this->db->dbprefix('purchaserequests');
        $posts_table = $this->db->dbprefix('posts');

        $notification_settings = $this->db->query("SELECT * 
        ...
        $pv_id = get_array_value($options, "pv_id");
        ...
        //find all approvers
        if (in_array("estimates_approvers", $notify_to_terms) && $invoice_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.role_id IN (SELECT $role_perm_table.role_id FROM $role_perm_table WHERE $role_perm_table.table_name='estimates' AND $role_perm_table.prove_row>0)
                      )
                    ) ";
        }
        ...
        //START PURCHASEREQUESTS
        if (in_array("purchaserequest_approvers", $notify_to_terms) && $pr_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.role_id IN (SELECT $role_perm_table.role_id FROM $role_perm_table WHERE $role_perm_table.table_name='purchaserequests' AND $role_perm_table.prove_row>0)
                      )
                    ) ";
        }
        //END PURCHASEREQUESTS

        if (in_array("payment_vouchers_approvers", $notify_to_terms) && $pv_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.role_id IN (SELECT $role_perm_table.role_id FROM $role_perm_table WHERE $role_perm_table.table_name='payment_vouchers' AND $role_perm_table.prove_row>0)
                      )
                    ) ";
        }
        ...
        //find all approvers
        if (in_array("invoice_approvers", $notify_to_terms) && $invoice_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.role_id IN (SELECT $role_perm_table.role_id FROM $role_perm_table WHERE $role_perm_table.table_name='invoices' AND $role_perm_table.prove_row>0)
                      )
                    ) ";
        }
        ...
        if ($client_id || $lead_id) {
            ...
            $client_info = $this->db->query("SELECT $clients_table.* FROM $clients_table WHERE $clients_table.id=$client_or_lead_id")->row();
        }
