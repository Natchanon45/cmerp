...
class Credit_notes_m extends MY_Model {
    ...
    function getIndexDataSetHTML($cnrow){
        ...
        $data = [
                    ...
                    "<a data-post-id='".$cnrow->id."' data-action-url='".get_uri("invoices/addedit")."' data-act='ajax-modal' class='edit'><i class='fa fa-pencil'></i></a><a data-id='".$cnrow->id."' data-action-url='".get_uri("invoices/delete_doc")."' data-action='delete' class='delete'><i class='fa fa-times fa-fw'></i></a>"
                ];
    ...
    function updateStatus(){
        ...
        if($updateStatusTo == "R"){
            ...
            $db->insert("receipt", [
                                        ...
                                        "project_id"=>$cnrow->project_id,
                                        "client_id"=>$cnrow->client_id,
                                        "sub_total_before_discount"=>$cnrow->sub_total_before_discount,
                                        "discount_type"=>$cnrow->discount_type,
                                        "discount_percent"=>$cnrow->discount_percent,
                                        "discount_amount"=>$cnrow->discount_amount,
                                        "sub_total"=>$cnrow->sub_total,
                                        "vat_inc"=>$cnrow->vat_inc,
                                        "vat_percent"=>$cnrow->vat_percent,
                                        "vat_value"=>$cnrow->vat_value,
                                        "total"=>$cnrow->total,
                                        "wht_inc"=>$cnrow->wht_inc,
                                        "wht_percent"=>$cnrow->wht_percent,
                                        "wht_value"=>$cnrow->wht_value,
                                        "payment_amount"=>$cnrow->payment_amount,
                                        "remark"=>$cnrow->remark,
                                        "created_by"=>$this->login_user->id,
                                        "created_datetime"=>date("Y-m-d H:i:s"),
                                        "status"=>"W",
            ...
            if(empty(!$cnrows)){
                foreach($cnrows as $cnrow){
                    $db->insert("receipt_items", [
                                                        ...
                                                        "product_id"=>$cnrow->product_id,
                                                        "product_name"=>$cnrow->product_name,
                                                        "product_description"=>$cnrow->product_description,
                                                        "quantity"=>$cnrow->quantity,
                                                        "unit"=>$cnrow->unit,
                                                        "price"=>$cnrow->price,
                                                        "total_price"=>$cnrow->total_price,
                                                        "sort"=>$cnrow->sort
                                                    ]);
                }
            }
