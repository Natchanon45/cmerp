...
class Expenses_model extends Crud_model
{
    ...
    function get_details($options = array(), $getRolePermission = array())
    {
        ...
        $sql = "
            SELECT $expenses_table.*, $expense_categories_table.title as category_title, 
                ...
                $clients_table.currency $select_custom_fields 
            FROM $expenses_table 
        ...
        if (!empty($options['id'])) {
            $filters['WHERE'][] = "" . $expenses_table . ".id = " . $options['id'] . "";
            if ($where) {
                $filters['WHERE'][] = " 1 " . $where;
            }
            $sql = gencond_($sql, $filters);
        } else {
            $getRolePermission['filters'] = isset($getRolePermission['filters']) ? $getRolePermission['filters'] : [];
            if ($where) {
                $getRolePermission['filters']['WHERE'][] = " 1 " . $where;
            }
    ...
    function get_income_expenses_info()
    {
        ...
        $sql1 = "
            SELECT SUM($invoice_payments_table.amount) as total_income 
        ...
        $sql2 = "
            SELECT SUM($expenses_table.amount + IFNULL(tax_table.percentage, 0) / 100 * IFNULL($expenses_table.amount, 0) + IFNULL(tax_table2.percentage, 0) / 100 * IFNULL($expenses_table.amount, 0)) AS total_expenses 
            FROM $expenses_table 
    ...
    function get_yearly_expenses_chart($year, $project_id = 0)
    {
        ...
        $sql = "
            SELECT SUM($expenses_table.amount + IFNULL(tax_table.percentage, 0) / 100 * IFNULL($expenses_table.amount, 0) + IFNULL(tax_table2.percentage, 0) / 100 * IFNULL($expenses_table.amount, 0)) AS total, MONTH($expenses_table.expense_date) AS month 
            FROM $expenses_table 
            LEFT JOIN (
