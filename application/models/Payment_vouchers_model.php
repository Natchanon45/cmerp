...
class Payment_vouchers_model extends Crud_model {
	...
	function get_invoice_total_summary( $invoice_id = 0, $order_id = 0 ) {
        ...
        $item_sql = "        
        SELECT SUM(amount) AS invoice_subtotal FROM payment_voucher_payments WHERE payment_vouchers_id = $invoice_id AND deleted=0  GROUP BY payment_vouchers_id";
        ...
        WHERE $payment_voucher_payments_table.deleted=0 AND $payment_voucher_payments_table.payment_vouchers_id=$invoice_id";
        $payment = $this->db->query($payment_sql)->row();

        $invoice_sql = "
			SELECT $payment_vouchers_table.*, tax_table.percentage AS tax_percentage, tax_table.title AS tax_name,
        ...
        $result = new stdClass();
        $result->invoice_subtotal = 0;
        $result->tax_percentage = 0;
        $result->tax_percentage2 = 0;
        ...
        $result->vat3 = $result->total_paid * ($invoice->tax_percentage3 / 100);

        $result->tol = ($result->invoice_total) - ($result->vat3);

        $result->tolvat7 = $result->tol * 0.07;
        
        $result->sumvat7 = $result->tolvat7 + $result->tol;

        $result->tolvat3 = $result->tol * 0.03;

        $result->sumvat3 = $result->sumvat7 - $result->tolvat3;

        // print_r($result->total_paid * ($invoice->tax_percentage3 / 100));exit;
    ...
    function invoice_statistics($options = array()) {
        ...
        if ($client_id) {
            ...
            $payments_where = " AND $payment_voucher_payments_table.invoice_id IN(SELECT $payment_vouchers_table.id FROM $payment_vouchers_table WHERE $payment_vouchers_table.deleted=0 $invoices_where)";
        }
        ...
        $payments = "SELECT SUM($payment_voucher_payments_table.amount) AS total, MONTH($payment_voucher_payments_table.payment_date) AS month
            FROM $payment_voucher_payments_table
            LEFT JOIN $payment_vouchers_table ON $payment_vouchers_table.id=$payment_voucher_payments_table.invoice_id    
        ...
        $invoices = "SELECT SUM(total) AS total, MONTH(due_date) AS month FROM (SELECT $invoice_value_calculation_query AS total ,$payment_vouchers_table.due_date
            ...
            LEFT JOIN (SELECT invoice_id, SUM(total) AS invoice_value FROM $payment_voucher_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id = $payment_vouchers_table.id 
    ...
    function get_details($options = array(), $getRolePermission = array() ) {
        ...
        $sql = "
			SELECT 
				$payment_vouchers_table.id, $payment_vouchers_table.doc_no,$payment_vouchers_table.created_by, 
				
				$payment_vouchers_table.user_id, $payment_vouchers_table.client_id, $payment_vouchers_table.project_id, $payment_vouchers_table.bill_date, $payment_vouchers_table.due_date, $payment_vouchers_table.note, $payment_vouchers_table.labels, $payment_vouchers_table.last_email_sent_date, 
				
				 
				
				IF( pt.id IS NULL,  $payment_vouchers_table.status, 'อนุมัติ' ) as status,
				
				$payment_vouchers_table.tax_id, 
				$payment_vouchers_table.tax_id2, $payment_vouchers_table.tax_id3, $payment_vouchers_table.recurring, $payment_vouchers_table.recurring_invoice_id, $payment_vouchers_table.repeat_every, $payment_vouchers_table.repeat_type, $payment_vouchers_table.no_of_cycles, $payment_vouchers_table.next_recurring_date, $payment_vouchers_table.no_of_cycles_completed, $payment_vouchers_table.due_reminder_date, $payment_vouchers_table.recurring_reminder_date, $payment_vouchers_table.discount_amount, $payment_vouchers_table.discount_amount_type, $payment_vouchers_table.discount_type, $payment_vouchers_table.cancelled_at, $payment_vouchers_table.cancelled_by, $payment_vouchers_table.created_by, $payment_vouchers_table.files, 
				$payment_vouchers_table.deleted,
				$clients_table.currency, 
				$clients_table.currency_symbol, 
			...
			FROM $payment_vouchers_table 
			LEFT JOIN prove_table pt ON $payment_vouchers_table.id = pt.doc_id AND pt.tbName = 'payment_vouchers'
			...
			LEFT JOIN (
				...
				FROM $payment_voucher_payments_table 
				WHERE deleted = 0 GROUP BY invoice_id ) AS payments_table ON payments_table.invoice_id = $payment_vouchers_table.id 
			LEFT JOIN ( 
				SELECT 
					invoice_id, 
				...
				GROUP BY 
					invoice_id 
			) AS items_table ON items_table.invoice_id = $payment_vouchers_table.id 
			$join_custom_fieds
			[WHERE]
		";
