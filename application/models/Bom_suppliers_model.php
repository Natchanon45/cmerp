...
class Bom_suppliers_model extends Crud_model
{
    ...
    function dev2_getMaterialDropdown()
    {
        ...
        foreach ($result as $item) {
            if (isset($item->production_name) && !empty($item->production_name)) {
                $item->material_name = str_replace("\"", "`", $item->name . ' - ' . $item->production_name);
            } else {
    ...
    function dev2_getSupplierDropdownWithCode()
    {
        ...
        foreach ($result as $item) {
            if ($item->code_supplier) {
                $data[] = array(
                    "id" => $item->id,
                    "text" => $item->code_supplier . ' - ' . $item->company_name
                );
            } else {
                $data[] = array(
                    "id" => $item->id,
    ...
    function dev2_get_dropdown_list()
    {
        $list = array();
