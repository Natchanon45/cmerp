...
class Invoices_m extends MY_Model {
    ...
    function getIndexDataSetHTML($qrow){
        ...
        $data = [
                    "<a href='".get_uri("invoices/view/".$qrow->id)."'>".$qrow->doc_number."</a>",
                    ...
                    "<a data-post-id='".$qrow->id."' data-action-url='".get_uri("invoices/addedit")."' data-act='ajax-modal' class='edit'><i class='fa fa-pencil'></i></a><a data-id='".$qrow->id."' data-action-url='".get_uri("invoices/delete_doc")."' data-action='delete' class='delete'><i class='fa fa-times fa-fw'></i></a>"
                ];
    ...
    function validateDoc(){
        ...
        if ($this->form_validation->run() == FALSE){log_message("error", 1);
            $this->data["status"] = "validate";
            if(form_error('invoice_date') != null) $this->data["messages"]["invoice_date"] = form_error('invoice_date');
    ...
    function saveDoc(){
        ...
        $docId = $this->json->doc_id;
        $doc_date = converDate($this->json->invoice_date);
        $credit = $this->json->credit;
        $due_date = converDate($this->json->due_date);
