...
/**
 * copy a directory to another directory format_to_datetime
 * 
 * @param string $src
 * @param string $dst
 * @return void
...
if (!function_exists('get_store_item_image')) {
    ...
    function get_store_item_image($files) {
        ...
        if ($files && is_array($files) && count($files)) {
            ...
            return get_source_url_of_file($first_file, get_setting("timeline_file_path"));
        } else {
...
function copy_files($files = [], $target_path = "", $related_to = "") {
	$ci = get_instance();
    $filename_prefix = $related_to . "_" . uniqid("file") . "-";
	...
	if ($files) {
        $temp_file_path = get_setting("timeline_file_path");
        $target_path = getcwd() . '/' . $temp_file_path;
        if (!is_dir($target_path)) {
            if (!mkdir($target_path, 0777, true)) {
                die('Failed to create file folders.');
		...
		foreach($files as $key => $file) {
            $sour_file = $target_path . $file['file_name'];
            $target_file = $target_path . '_new_'.$file['file_name'];
            // echo 'copy '.$sour_file.' to '.$target_file."<br />";
            @copy($sour_file, $target_file);
			$files_data[] = array(
				"file_name" => '_new_'.$file['file_name'],
				"file_size" => get_array_value($file, $key),
				"file_id" => get_array_value($file, "file_id"),
				"service_type" => get_array_value($file, "service_type")
			);
		}
