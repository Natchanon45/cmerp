...
if (!function_exists('get_notification_config')) {
    ...
    function get_notification_config($event = "", $key = "", $info_options = array()) {
        ...
        $invoice_view_link = function($options) {
            $url = "";
            if (isset($options->invoice_id)) {
                $url = get_uri("invoices/view/" . $options->invoice_id);
            }
        ...
        $estimate_view_link = function($options) {
            $url = "";
            if (isset($options->estimate_id)) {
        ...
        $purchaserequest_link = function($options) {
            $url = "";
            if (isset($options->pr_id)) {
                $url = get_uri("purchaserequests/view/" . $options->pr_id);
            }

            return array("url" => $url);
        };

        $payment_voucher_link = function($options) {
            $url = "";
            if (isset($options->pv_id)) {
                $url = get_uri("payment_vouchers/view/" . $options->pv_id);
            }
        ...
        $stock_link = function($options) {
            $url = "";
            if (isset($options->post_id)) {
                $url = get_uri("stock/notification/" . $options->post_id);
            }
            return array("url" => $url);
        };
        ...
        $events = array(
            ...
            "new_invoice_created" => array(
                "notify_to" => array("invoice_approvers"),
                "info" => $invoice_view_link
            ),
            ...
            "new_estimate_created" => array(
                "notify_to" => array("estimate_approvers"),
                "info" => $estimate_view_link
            ),
            ...
            "new_purchaserequest_created" => array(
                "notify_to" => array("purchaserequest_approvers"),
                "info" => $purchaserequest_link
            ),
            "new_payment_voucher_created" => array(
                "notify_to" => array("payment_voucher_approvers"),
                "info" => $payment_voucher_link
            ),
...
if (!function_exists('send_notification_emails')) {
    ...
    function send_notification_emails($notification_id, $email_notify_to, $extra_data = array()) {
        ...
        if ($notification->category == "ticket" && $notification->event !== "ticket_assigned") {
            ...
            $comment_info = $ci->Ticket_comments_model->get_details($comments_options)->row();
        ...
        if ($notification_multiple_tasks_user_wise && ($notification->event == "project_task_deadline_pre_reminder" || $notification->event == "project_task_reminder_on_the_day_of_deadline" || $notification->event == "project_task_deadline_overdue_reminder")) {
            ...
            foreach ($notification_multiple_tasks_user_wise as $user_id => $tasks) {
                ...
                $table = $ci->load->view("projects/tasks/notification_multiple_tasks_table", array("tasks" => $tasks), true);
...
if (!function_exists('send_push_notifications')) {
    ...
    function send_push_notifications($event, $push_notify_to, $user_id = 0, $notification_id = 0) {
        ...
        if ($pusher_app_id && $pusher_key && $pusher_secret && $pusher_cluster) {
            ...
            $message = lang("notification_" . $event);
            if ($notification_id) {
                $to_user_name = $ci->Notifications_model->get_to_user_name($notification_id);
                if ($to_user_name) {
                    $message = sprintf(lang("notification_" . $event), $to_user_name);
                }
            }
    ...
    function get_notification_multiple_tasks_data($tasks, $event) {
        ...
        foreach ($tasks as $task) {
            ...
            if (!in_array($task->project_id, $project_ids) && in_array("project_members", $notify_to_terms_array)) {
                ...
                $project_members = $ci->Project_members_model->get_details($options)->result();
    ...
    function send_slack_notification($event, $user_id = 0, $notification_id = 0, $webhook_url = "") {
        if ($webhook_url) {
            ...
            if ($notification_id) {
                ...
                $notification_description = $ci->load->view("notifications/notification_description_for_slack", array("notification" => $notification_info), true);
            }
