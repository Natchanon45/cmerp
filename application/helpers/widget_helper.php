...
/**
 * get receipt_taxinvoice statistics widget
 * @param boolean $return_as_data
 * @return html
 */
if (!function_exists('receipt_taxinvoice_statistics_widget')) {

    function receipt_taxinvoice_statistics_widget($return_as_data = false, $options = array()) {
        $ci = get_instance();
        ...
        $options["currency_symbol"] = $currency_symbol;
        $info = $ci->Receipt_taxinvoices_model->receipt_taxinvoice_statistics($options);

        $payments = array();
        $payments_array = array();

        $receipt_taxinvoices = array();
        $receipt_taxinvoices_array = array();

        for ($i = 1; $i <= 12; $i++) {
            $payments[$i] = 0;
            $receipt_taxinvoices[$i] = 0;
        }

        foreach ($info->payments as $payment) {
            $payments[$payment->month] = $payment->total;
        }
        foreach ($info->receipt_taxinvoices as $receipt_taxinvoice) {
            $receipt_taxinvoices[$receipt_taxinvoice->month] = $receipt_taxinvoice->total;
        }
        ...
        foreach ($receipt_taxinvoices as $key => $receipt_taxinvoice) {
            $receipt_taxinvoices_array[] = array($key, $receipt_taxinvoice);
        }

        $view_data["payments"] = json_encode($payments_array);
        $view_data["receipt_taxinvoices"] = json_encode($receipt_taxinvoices_array);
        $view_data["currencies"] = $info->currencies;
        $view_data["currency_symbol"] = $currency_symbol;

        return $ci->load->view("receipt_taxinvoices/receipt_taxinvoice_statistics_widget/index", $view_data, $return_as_data);
    }
...
if (!function_exists('count_new_tickets')) {
    ...
    function count_new_tickets($ticket_types = "", $show_assigned_tickets_only_user_id = 0) {
        $ci = get_instance();
        return $ci->Tickets_model->count_new_tickets($ticket_types, $show_assigned_tickets_only_user_id);
    }
...
if (!function_exists('active_members_and_clients_widget')) {
    ...
    function active_members_and_clients_widget($user_type = "", $return_as_data = false, $show_own_clients_only_user_id = "") {
        $ci = get_instance();
...
if (!function_exists('total_clients_widget')) {
    ...
    function total_clients_widget($return_as_data = false, $show_own_clients_only_user_id = "") {
        $ci = get_instance();
        $view_data["total"] = $ci->Clients_model->count_total_clients($show_own_clients_only_user_id);
...
if (!function_exists('total_contacts_widget')) {
    ...
    function total_contacts_widget($return_as_data = false, $show_own_clients_only_user_id = "") {
        $ci = get_instance();
        $view_data["total"] = $ci->Users_model->count_total_contacts($show_own_clients_only_user_id);
        return $ci->load->view("clients/total_contacts_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('active_members_on_projects_widget')) {
    ...
    function active_members_on_projects_widget($return_as_data = false) {
        $ci = get_instance();

        $view_data["users_info"] = $ci->Timesheets_model->active_members_on_projects()->result();

        return $ci->load->view("team_members/active_members_on_projects_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('open_tickets_list_widget')) {
    ...
    function open_tickets_list_widget($return_as_data = false) {
        $ci = get_instance();

        if ($ci->login_user->user_type == "client") {
            $view_data["client_id"] = $ci->login_user->client_id;
            return $ci->load->view("clients/tickets/open_tickets_list_widget", $view_data, $return_as_data);
        } else {
            return $ci->load->view("tickets/open_tickets_list_widget", "", $return_as_data);
        }
    }
